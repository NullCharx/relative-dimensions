...
public class S2CRemainingTime {
    ...
    public void handle(CustomPayloadEvent.Context ctx) {
        ...
        long remainingTimeSeconds = es.nullbyte.charmiscmods.charspvp.timenpvpstate.GUI.LocalTimeState.dailyTL - remainingTime; //Calculate player remining time with local dauly limit
        if (remainingTime == 45296) { //Set untoggled time.
            remainingTimeSeconds = 45296;
        }
        //Get the playerTimeTracker
        es.nullbyte.charmiscmods.charspvp.timenpvpstate.GUI.LocalTimeState.localtimers.put(client.player.getUUID(), remainingTimeSeconds); //Syncronize
