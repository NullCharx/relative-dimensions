...
/**
 * Need an override cuz aberrant grass becomes normal dirt when placed below a block
 */
public class AberrantGrass extends GrassBlock {
    public AberrantGrass(Properties p_53685_) {
        super(p_53685_);
    }
    ...
    public void randomTick(BlockState p_222508_, ServerLevel p_222509_, BlockPos p_222510_, RandomSource p_222511_) {
        if (!canBeGrass(p_222508_, p_222509_, p_222510_)) {
            ...
            p_222509_.setBlockAndUpdate(p_222510_, BlockInit.ABERRANT_DIRT.get().defaultBlockState());
        } else {
            ...
            if (p_222509_.getMaxLocalRawBrightness(p_222510_.above()) >= 9) {
                ...
                for(int i = 0; i < 4; ++i) {
                    ...
                    if (p_222509_.getBlockState(blockpos).is(BlockInit.ABERRANT_DIRT.get()) && canPropagate(blockstate, p_222509_, blockpos)) {
                        p_222509_.setBlockAndUpdate(blockpos, blockstate.setValue(SNOWY, Boolean.valueOf(p_222509_.getBlockState(blockpos.above()).is(BlockInit.ABERRANT_SNOWY_GRASS.get()))));
                    }
                }
            }
